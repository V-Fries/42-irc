...
const ft::String &Command::getCommand() const { return _command; }

const ft::Vector<ft::String> &Command::getArgs() const { return _args; }
...
static ft::String getCommand(const ft::String &rawCommand,
                             ft::String::const_iterator &it) {
  while (it != rawCommand.end() && isspace(*it))
    ++it;
  ...
  ft::String command = ft::String::getFirstWord(it, rawCommand.end(), ": ");
  it += command.length();
  return (command);
}
...
ft::Vector<ft::String> getArgs(const ft::String &rawCommand,
  ...
  while (currIterator != rawCommand.end() && *currIterator != ':')
    ++currIterator;
